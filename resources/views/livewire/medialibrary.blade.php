            ...
            <div class=" border-gray-200 whitespace-nowrap text-sm flex gap-8 justify-end items-center w-full">
                ...
                <div x-data="{ open: @entangle('FormFolder') }" class="flex justify-end gap-4">
                    ...
                    <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
                        ...
                        <div
                            class="border-gray-200 whitespace-nowrap text-sm flex gap-8 justify-end items-center w-full">
                            <div class="flex flex-col items-end gap-2">
                                <div class="flex justify-end gap-4">
                                    <label for="file-upload" class="btn btn-primary cursor-pointer"
                                        :class="uploading ? 'opacity-50 pointer-events-none' : ''">
                                        {{-- Spinner: Zeigt sich während des Uploads --}}
                                        <div wire:loading wire:target="files">
                                            <svg class="animate-spin h-5 w-6" xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        {{-- Icon: Verschwindet während des Uploads --}}
                                        <x-tabler-upload stroke-width="1.5" wire:loading.remove
                                            wire:target="files" />

                                        <span>{{ __('Add file') }}</span>
                                    </label>

                                    {{-- WICHTIG: wire:model bindet das File-Input direkt an Livewire --}}
                                    <input type="file" id="file-upload" wire:model="files" multiple class="hidden" />
                                </div>

                                {{-- Fortschrittsbalken (optional) --}}
                                <div x-show="uploading"
                                    class="w-64 bg-gray-200 rounded-full h-1.5 mt-2 overflow-hidden">
                                    <div class="bg-blue-600 h-1.5 rounded-full transition-all duration-150"
                                        :style="`width: ${progress}%; text-align: right;`"></div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <x-kompass::offcanvas :w="'w-1/3'" class="p-8 grid gap-4">
                <x-slot name="body">
                    ...
                    <div class="modal-footer mt-4 flex gap-4">
                        ...
                        <button wire:click="selectItem({{ $iditem }}, 'delete')" “
