<div>
        ...
        <x-kompass::offcanvas :w="'w-2/6'">
            <x-slot name="body">
                ...
                <div class="grid grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button type="button" wire:click="$set('color', 'primary')" class="w-8 h-8 rounded-full border-2 {{ $color === 'primary' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #3b82f6;" title="primary"></button>
                            <button type="button" wire:click="$set('color', 'secondary')" class="w-8 h-8 rounded-full border-2 {{ $color === 'secondary' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #a855f7;" title="secondary"></button>
                            <button type="button" wire:click="$set('color', 'accent')" class="w-8 h-8 rounded-full border-2 {{ $color === 'accent' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #f472b6;" title="accent"></button>
                            <button type="button" wire:click="$set('color', 'info')" class="w-8 h-8 rounded-full border-2 {{ $color === 'info' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #06b6d4;" title="info"></button>
                            <button type="button" wire:click="$set('color', 'success')" class="w-8 h-8 rounded-full border-2 {{ $color === 'success' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #22c55e;" title="success"></button>
                            <button type="button" wire:click="$set('color', 'warning')" class="w-8 h-8 rounded-full border-2 {{ $color === 'warning' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #eab308;" title="warning"></button>
                            <button type="button" wire:click="$set('color', 'error')" class="w-8 h-8 rounded-full border-2 {{ $color === 'error' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #ef4444;" title="error"></button>
                            <button type="button" wire:click="$set('color', 'neutral')" class="w-8 h-8 rounded-full border-2 {{ $color === 'neutral' ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105' }}" style="background-color: #71717a;" title="neutral"></button>
                        </div>
                ...
                <div class="mt-4">
                    ...
                    <div class="relative">
                        <input type="text" wire:model="iconSearch" wire:keyup.debounce.300ms="loadIcons" placeholder="{{ __('Search icon...') }}" class="input input-bordered w-full pl-10" />
                        <x-tabler-search class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-base-content/50" />
                    </div>
                        ...
                        <div class="mt-2 max-h-40 overflow-y-auto border border-base-300 rounded bg-base-100">
                            <div class="grid grid-cols-6 gap-1 p-2">
                                @foreach($filteredIcons as $iconItem)
                                    <button wire:click="selectIcon('{{ $iconItem['name'] }}')" class="p-2 hover:bg-base-200 rounded flex justify-center transition-colors">
    ...
    <div class="flex flex-col">
        <div class="border-gray-200 whitespace-nowrap text-sm flex gap-8 justify-between items-center">
            <div class="w-full">
                <x-kompass::form.input type="text" name="search" wire:model.live="search" placeholder="{{ __('Search categories...') }}" />
            </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap bg-base-100">
                                        <div class="flex justify-end items-center gap-1">
                                            <span wire:click="selectItem({{ $category->id }}, 'edit')" class="flex justify-center cursor-pointer">
                                                <x-tabler-edit class="stroke-blue-500" />
                                            </span>
                                            <span wire:click="selectItem({{ $category->id }}, 'delete')" class="flex justify-center cursor-pointer">
