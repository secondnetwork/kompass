        ...
        <figure class="relative aspect-video overflow-hidden">
            @if ($file->type == 'folder')
                <div class="flex items-center justify-center w-full h-full bg-base-200">
                    <x-tabler-folder class="w-16 h-16 opacity-40 group-hover:scale-110 transition-transform duration-300" />
                </div>
            @else
            ...
            <div class="absolute top-2 right-2 flex gap-1">
                <div class="badge badge-neutral badge-xs opacity-80 uppercase">{{ $file->extension ?? ($file->type == 'folder' ? __('Folder') : '') }}</div>
            </div>
        </figure>
        <div class="card-body p-3">
            <div class="flex justify-between items-start gap-2">
                ...
                <div class="absolute top-2 left-2 flex gap-1">
                    @if ($file->type != 'folder')
                        <button wire:click.stop="selectField({{ $file->id }})" class="badge badge-primary cursor-pointer selectField" title="{{ __('Use file') }}">
                            <x-tabler-square-plus class="w-4 h-4 stroke-current" />
                        </button>
                    @endif
                    <button wire:click.stop="select" class="badge badge-primary cursor-pointer" title="{{ __('Edit Meta') }}">
